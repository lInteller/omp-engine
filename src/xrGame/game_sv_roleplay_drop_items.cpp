...
void game_sv_roleplay::FillDeathActorRejectItems(CSE_ActorMP *actor, xr_vector<CSE_Abstract*> & to_reject)
{
	R_ASSERT(actor);
...
BOOL game_sv_roleplay::OnTouchPlayersBag(CSE_ActorMP *actor, CSE_Abstract *item)
{
	R_ASSERT(actor);
	R_ASSERT(item);
...
void game_sv_roleplay::OnDetachPlayersBag(CSE_ActorMP *actor, CSE_Abstract *item)
{
	R_ASSERT(actor);
	R_ASSERT(item);
